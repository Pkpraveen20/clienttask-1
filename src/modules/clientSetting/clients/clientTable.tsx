...
export default function ClientTable() {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: ({ id, status }: { id: number; status: string }) =>
      axios.patch(`http://localhost:3000/clients/${id}`, { status }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["clients"] }),
  });
  ...
  function getFilteredSortedClients(clients: any[]) {
  ...
  useEffect(() => {
    if (data && Array.isArray(data)) {
      data.forEach((client: any) => {
        const shouldBeActive = getStatus(client) === "Active";
        if (
          (shouldBeActive && client.status !== "Active") ||
          (!shouldBeActive && client.status !== "Inactive")
        ) {
  ...
  const filteredClients = getFilteredSortedClients(data || []);
