...
export default function PermissionGroupView() {
  const { id } = useParams({ from: "/permissiongroup/$id" });
  ...
  const {
    ...
    queryFn: async () => {
      const res = await axios.get(`http://localhost:3000/permissiongroup/${id}`);
      return res.data;
    },
  });
  ...
  if (error || !permissiongroup) {
    return (
      <div className="text-center py-8">
        ...
        <button
          onClick={() => navigate({ to: "/permissiongroup" })}
      ...
      <div className="bg-white rounded-lg shadow-lg overflow-hidden">
        <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-6">
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
              ...
              <p className="text-blue-800 text-lg">{permissiongroup.permissiongroupname}</p>
            </div>
            ...
            <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
              ...
              <p className="text-gray-800 text-lg">{permissiongroup.permissiongroupstatus}</p>
            </div>
        ...
        <div className="bg-gray-50 px-8 py-4 border-t border-gray-200">
          <div className="flex justify-between items-center">
            <p className="text-sm text-gray-600"> ID: {permissiongroup.id}</p>
            <button
              onClick={() => navigate({ to: "/permissiongroup" })}
