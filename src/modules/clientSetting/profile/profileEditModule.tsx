...
export default function ProfileEditModal({
  ...
  useEffect(() => {
    if (profile) {
      setForm({
        ...
        profileclient: Array.isArray(profile.profileclient)
          ? profile.profileclient.map((p: string) => ({
              value: p,
              label: p,
            }))
          : [],
        profilefunctional: Array.isArray(profile.profilefunctional)
          ? profile.profilefunctional.map((p: string) => ({
              value: p,
              label: p,
            }))
          : [],
        profilerole: Array.isArray(profile.profilerole)
          ? profile.profilerole.map((p: string) => ({
              value: p,
              label: p,
            }))
          : [],
        profilepermission: Array.isArray(profile.profilepermission)
          ? profile.profilepermission.map((p: string) => ({
              value: p,
              label: p,
            }))
          : [],
        profilepermissiongroup: Array.isArray(profile.profilepermissiongroup)
          ? profile.profilepermissiongroup.map((p: string) => ({
              value: p,
              label: p,
            }))
          : [],
  ...
  const updateProfile = useMutation({
    mutationFn: async (updateProfile: typeof form) => {
      return axios.put(`http://localhost:3000/profile/${id}`, updateProfile);
    },
    onSuccess: () => {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    updateProfile.mutate(formattedForm);
  }
