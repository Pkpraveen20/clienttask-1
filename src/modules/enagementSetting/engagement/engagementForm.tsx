...
function getDisabledSlots(slots: DateTimeSlot[]) {
  const all: DateTimeSlot[] = [];

  slots.forEach((slot) => {
    const tz = timezoneMap[slot.timezone] || "UTC";
    const slotDate = DateTime.fromFormat(
      `${slot.date} ${slot.time}`,
      "yyyy-MM-dd HH:mm",
      { zone: tz }
    ...
    if (slotDate.isValid) {
      ...
      for (let minutes = -30; minutes <= 30; minutes += 15) {
        if (minutes === 0) continue;
        const buffered = slotDate.plus({ minutes });
        all.push({
          ...slot,
          id: `${slot.id}-buffer-${minutes}`,
          date: buffered.toFormat("yyyy-MM-dd"),
          time: buffered.toFormat("HH:mm"),
        });
      }
    }
  });
            ...
            <div className=" rounded-lg p-2">
              ...
              <div className="flex justify-end mb-3">
                <button
                  ...
                  className="mt-3 bg-blue-600 text-white px-4 py-2 rounded disabled:opacity-500 "
                  disabled={
                    !slotPicker.date ||
                    !slotPicker.time ||
                    !slotPicker.timezone ||
              ...
              <div className="md:w-3/3 border border-gray-300 rounded-lg p-4 h-auto bg-white ">
